...
async function seed(): Promise<void>
{
    ...
    // 3. build a set of "uid:startupId" investor pairs so we only assign
    //    is_private: true to users who actually hold tokens in that startup.
    //    If seed:investments has not been run yet, all questions will be public.
    const investorPairs = new Set<string>();
    const investmentsSnap = await db.collectionGroup('investments').get();
    for (const doc of investmentsSnap.docs)
    {
        // path: users/{uid}/investments/{docId} — parent.parent.id is the uid
        const uid       = doc.ref.parent.parent!.id;
        const startupId = (doc.data().startup_id as string | undefined) ?? '';
        if (startupId) investorPairs.add(`${uid}:${startupId}`);
    }
    console.log(`  ${investorPairs.size} par(es) investidor-startup carregados.\n`);

    // 4. build the combined template pool once
    ...
    for (const startup of startups)
    {
        ...
        for (let qi = 0; qi < selected.length; qi++)
        {
            ...
            // is_private: ~22% of answered questions can be private, but only if the
            // author is actually an investor in this startup — enforcing the same rule
            // as onSendQuestion on the backend.
            const isInvestor = investorPairs.has(`${user.uid}:${startup.id}`);
            const isPrivate  = hasAnswer && (slotHash % 9 < 2) && isInvestor;
        ...
        const privateCount = selected.filter((t, qi) =>
        {
            ...
            const userIdx2 = (h + qi * 13) % users.length;
            const inv      = investorPairs.has(`${users[userIdx2].uid}:${startup.id}`);
            return ha && (sh % 9 < 2) && inv;
        }).length;
