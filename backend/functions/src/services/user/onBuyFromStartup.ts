...
export async function handleOnBuyFromStartup(request: CallableRequest)
{
    try
    {
        ...
        // Generate the order ref before the transaction so the ID is known
        // but the write happens atomically inside it.
        ...
        ({totalAmount, completedAt} = await db.runTransaction(async (tx) =>
        {
            ...
            // Create the order atomically inside the transaction as completed.
            // This guarantees the order only appears in the DB after the wallet
            // debit and investment update commit — eliminating the window where a
            // pending order with filled_quantity=0 could corrupt balance/token checks.
            tx.set(orderRef, {
                id:              orderRef.id,
                uid,
                startup_id:      startupId,
        ...
        {
            logger.warning(`Failed to record transaction for primary order "${orderRef.id}": ${txErr}`);
        }

        logger.info(`Primary order "${orderRef.id}" completed. User "${uid}" bought ${quantity} tokens of "${startupId}".`);
